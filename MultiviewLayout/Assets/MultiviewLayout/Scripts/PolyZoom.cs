...
namespace MultiViewLayout
{
    public class PolyZoom: PolyLayout
    {
        ...
        /// <summary>
        /// Solve all local branch overlap of the views
        /// </summary>
        public void SolveAllLocalBranchOverlaps()
        {
            foreach(ViewZoom v in Views())
            {
                if (IsChildrenOverlap(v))
                {
                    SolveChildrenOverlap(v);
                }
            }
        }
